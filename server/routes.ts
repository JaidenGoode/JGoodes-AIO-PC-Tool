...
try {
  ...
  if ($r -eq 0) { Write-Output "Standby memory purged successfully." } else { Write-Output "Completed (NTSTATUS 0x$('{0:X8}' -f $r))" }
        ...
        "nvidia-cp": { name: "NVIDIA Control Panel", command: String.raw`powershell -NoProfile -WindowStyle Hidden -Command "$drives=Get-PSDrive -PSProvider FileSystem -EA SilentlyContinue|Select-Object -Exp Root;$found=$null;foreach($d in $drives){$pp=@($d+'Program Files\NVIDIA Corporation\Control Panel Client\nvcplui.exe',$d+'Windows\System32\nvcplui.exe',$d+'Windows\SysWOW64\nvcplui.exe',$d+'Program Files (x86)\NVIDIA Corporation\Control Panel Client\nvcplui.exe');$f=$pp|Where-Object{Test-Path $_}|Select-Object -First 1;if($f){$found=$f;break}};if($found){Start-Process $found}else{try{Start-Process 'nvidia-controlpanel://' -EA Stop}catch{Start-Process 'https://www.nvidia.com/en-us/drivers/nvidia-control-panel/'}}"`, description: "Opens the NVIDIA Control Panel. Searches all drives. Falls back to NVIDIA download page if not found." },
        "nvidia-app": { name: "NVIDIA App", command: String.raw`powershell -NoProfile -WindowStyle Hidden -Command "$p=Get-Process -Name 'nvidia-app' -EA SilentlyContinue|Select-Object -First 1 -Exp Path;if($p){Start-Process $p;exit};$drives=Get-PSDrive -PSProvider FileSystem -EA SilentlyContinue|Select-Object -Exp Root;$found=$null;foreach($d in $drives){$f=Get-ChildItem @($d+'Program Files\NVIDIA Corporation',$d+'Program Files (x86)\NVIDIA Corporation') -Filter 'nvidia-app.exe' -Recurse -Depth 5 -EA SilentlyContinue|Select-Object -First 1 -Exp FullName;if($f){$found=$f;break}};if($found){Start-Process $found}else{Start-Process 'https://www.nvidia.com/en-us/software/nvidia-app/'}"`, description: "Opens the NVIDIA App. Checks running processes and all drives. Falls back to NVIDIA website if not installed." },
