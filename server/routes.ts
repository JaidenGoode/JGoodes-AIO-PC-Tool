...
try {
  ...
  if ($r -eq 0) { Write-Output "Standby memory purged successfully." } else { Write-Output "Completed (NTSTATUS 0x$('{0:X8}' -f $r))" }
        ...
        "nvidia-cp": { name: "NVIDIA Control Panel", command: String.raw`powershell -NoProfile -WindowStyle Hidden -Command "$p=@('C:\Program Files\NVIDIA Corporation\Control Panel Client\nvcplui.exe','C:\Windows\System32\nvcplui.exe','C:\Windows\SysWOW64\nvcplui.exe','C:\Program Files (x86)\NVIDIA Corporation\Control Panel Client\nvcplui.exe');$f=$p|Where-Object{Test-Path $_}|Select-Object -First 1;if($f){Start-Process $f}else{try{Start-Process 'nvidia-controlpanel://' -ErrorAction Stop}catch{Start-Process 'https://www.nvidia.com/en-us/drivers/nvidia-control-panel/'}}"`, description: "Opens the NVIDIA Control Panel. Falls back to NVIDIA download page if not found." },
        "nvidia-app": { name: "NVIDIA App", command: String.raw`powershell -NoProfile -WindowStyle Hidden -Command "$p=@('C:\Program Files\NVIDIA Corporation\NVIDIA app\nvidia-app.exe','C:\Program Files\NVIDIA Corporation\NVIDIA App\nvidia-app.exe','C:\Program Files\NVIDIA Corporation\NVIDIA App\NVIDIA App.exe');$f=$p|Where-Object{Test-Path $_}|Select-Object -First 1;if(-not $f){$f=Get-ChildItem 'C:\Program Files\NVIDIA Corporation' -Filter 'nvidia-app.exe' -Recurse -Depth 4 -ErrorAction SilentlyContinue|Select-Object -First 1 -ExpandProperty FullName};if($f){Start-Process $f}else{Start-Process 'https://www.nvidia.com/en-us/software/nvidia-app/'}"`, description: "Opens the NVIDIA App. Falls back to NVIDIA website if not installed." },
