...
try {
  ...
  if ($r -eq 0) { Write-Output "Standby memory purged successfully." } else { Write-Output "Completed (NTSTATUS 0x$('{0:X8}' -f $r))" }
        ...
        "nvidia-cp": { name: "NVIDIA Control Panel", command: String.raw`powershell -NoProfile -WindowStyle Hidden -Command "$regs=@('HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*');foreach($r in $regs){$e=Get-ItemProperty $r -EA SilentlyContinue|Where-Object{$_.DisplayName -match 'NVIDIA Control Panel'}|Select-Object -First 1;if($e){if($e.InstallLocation){$f=Get-ChildItem $e.InstallLocation -Filter 'nvcplui.exe' -Recurse -Depth 3 -EA SilentlyContinue|Select-Object -First 1 -Exp FullName;if($f){Start-Process $f;exit}};if($e.DisplayIcon){$i=($e.DisplayIcon -split ',')[0].Trim([char]34);if($i -and (Test-Path $i)){Start-Process $i;exit}}}};$drives=Get-PSDrive -PSProvider FileSystem -EA SilentlyContinue|Select-Object -Exp Root;foreach($d in $drives){$pp=@($d+'Program Files\NVIDIA Corporation\Control Panel Client\nvcplui.exe',$d+'Windows\System32\nvcplui.exe',$d+'Windows\SysWOW64\nvcplui.exe',$d+'Program Files (x86)\NVIDIA Corporation\Control Panel Client\nvcplui.exe');$f=$pp|Where-Object{Test-Path $_}|Select-Object -First 1;if($f){Start-Process $f;exit};$f=Get-ChildItem @($d+'Program Files',$d+'Program Files (x86)',$d+'Windows\System32',$d+'Windows\SysWOW64') -Filter 'nvcplui.exe' -Recurse -Depth 4 -EA SilentlyContinue|Select-Object -First 1 -Exp FullName;if($f){Start-Process $f;exit}};Start-Process 'https://www.nvidia.com/en-us/drivers/nvidia-control-panel/'"`, description: "Opens the NVIDIA Control Panel. Registry-first detection across all drives. Falls back to download page if not found." },
        "nvidia-app": { name: "NVIDIA App", command: String.raw`powershell -NoProfile -WindowStyle Hidden -Command "$regs=@('HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\*','HKLM:\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\*');foreach($r in $regs){$e=Get-ItemProperty $r -EA SilentlyContinue|Where-Object{$_.DisplayName -match 'NVIDIA App'}|Select-Object -First 1;if($e){if($e.InstallLocation){$f=Get-ChildItem $e.InstallLocation -Filter 'nvidia-app.exe' -Recurse -Depth 3 -EA SilentlyContinue|Select-Object -First 1 -Exp FullName;if($f){Start-Process $f;exit}};if($e.DisplayIcon){$i=($e.DisplayIcon -split ',')[0].Trim([char]34);if($i -and (Test-Path $i)){Start-Process $i;exit}}}};$drives=Get-PSDrive -PSProvider FileSystem -EA SilentlyContinue|Select-Object -Exp Root;foreach($d in $drives){$f=Get-ChildItem @($d+'Program Files',$d+'Program Files (x86)') -Filter 'nvidia-app.exe' -Recurse -Depth 4 -EA SilentlyContinue|Select-Object -First 1 -Exp FullName;if($f){Start-Process $f;exit}};Start-Process 'https://www.nvidia.com/en-us/software/nvidia-app/'"`, description: "Opens the NVIDIA App. Registry-first detection across all drives. Falls back to NVIDIA website if not installed." },
