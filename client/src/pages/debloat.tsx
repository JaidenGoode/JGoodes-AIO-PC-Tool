...
const CTT_PRESET_B64 = "";
...
export default function Debloat() {
  ...
  const importCttProfile = async () => {
    if (!window.electronAPI?.runScript) {
      toast({ title: "Desktop app required", description: "Requires the desktop .exe app.", variant: "destructive" });
      return;
    }
    setCttImportStatus("running");
    const script = [
      `$b64 = ''`,
      `$bytes = [System.Convert]::FromBase64String($b64)`,
      `$json = [System.Text.Encoding]::UTF8.GetString($bytes)`,
      `$dir = Join-Path $env:LOCALAPPDATA 'JGoode-AIO\\Tool'`,
      `if (-not (Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }`,
      `$path = Join-Path $dir 'CTT-preset.json'`,
      `[IO.File]::WriteAllText($path, $json)`,
      `$innerCmd = '$w = irm https://christitus.com/win; & ([scriptblock]::Create($w)) -Preset "' + $path + '"'`,
      `$enc = [System.Convert]::ToBase64String([System.Text.Encoding]::Unicode.GetBytes($innerCmd))`,
      `Start-Process powershell.exe -ArgumentList "-NoProfile", "-ExecutionPolicy", "Bypass", "-EncodedCommand", $enc -Verb RunAs -WindowStyle Normal`,
    ].join("\r\n");
    try {
      const r = await window.electronAPI.runScript(script);
      if (r.success) {
        setCttImportStatus("done");
        toast({ title: "WinUtil launched with preset", description: "Accept the UAC prompt — your preset is pre-loaded." });
      } else {
        setCttImportStatus("error");
        toast({ title: "Launch failed", description: "Could not launch WinUtil with preset.", variant: "destructive" });
      }
    } catch { setCttImportStatus("error"); }
    setTimeout(() => setCttImportStatus("idle"), 5000);
  };

  const importShutup10Profile = async () => {
    if (!window.electronAPI?.runScript) {
      toast({ title: "Desktop app required", description: "Requires the desktop .exe app.", variant: "destructive" });
      return;
    }
    setShutupImportStatus("downloading");
    const script = [
      `$ErrorActionPreference = 'SilentlyContinue'`,
      `$b64 = ''`,
      `$bytes = [System.Convert]::FromBase64String($b64)`,
      `$cfg = [System.Text.Encoding]::UTF8.GetString($bytes)`,
      `$dir = Join-Path $env:LOCALAPPDATA 'JGoode-AIO\\Tool'`,
      `if (-not (Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }`,
      `$cfgPath = Join-Path $dir 'JGoode-profile.cfg'`,
      `[IO.File]::WriteAllText($cfgPath, $cfg)`,
      `$exePath = $null`,
      `$cacheDest = Join-Path $dir 'OOSU10.exe'`,
      `foreach ($rp in @('HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\OOSU10.exe','HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\OOSU10.exe')) { if (Test-Path $rp) { $v = (Get-ItemProperty $rp -EA SilentlyContinue).'(default)'; if ($v -and (Test-Path $v)) { $exePath = $v; break } } }`,
      `if (-not $exePath) { foreach ($c in @("$env:ProgramFiles\\OO Software\\ShutUp10\\OOSU10.exe","\${env:ProgramFiles(x86)}\\OO Software\\ShutUp10\\OOSU10.exe","$env:LOCALAPPDATA\\Programs\\OO Software\\ShutUp10\\OOSU10.exe")) { if (Test-Path $c) { $exePath = $c; break } } }`,
      `if (-not $exePath -and (Test-Path $cacheDest) -and (Get-Item $cacheDest -EA SilentlyContinue).Length -ge 1048576) { $exePath = $cacheDest }`,
      `if (-not $exePath) { $ErrorActionPreference = 'Stop'; $wc = New-Object System.Net.WebClient; $wc.Headers.Add('User-Agent','Mozilla/5.0'); $wc.DownloadFile('https://dl5.oo-software.com/files/ooshutup10/OOSU10.exe',$cacheDest); $exePath = $cacheDest }`,
      `Start-Process -FilePath $exePath -ArgumentList $cfgPath -WindowStyle Normal`,
    ].join("\r\n");
    try {
      const r = await window.electronAPI.runScript(script);
      if (r.success) {
        setShutupImportStatus("done");
        toast({ title: "Profile imported", description: "ShutUp10++ opened with JGoode's profile. Review settings then click Apply." });
      } else {
        setShutupImportStatus("error");
        toast({ title: "Launch failed", description: "Could not import ShutUp10++ profile.", variant: "destructive" });
      }
    } catch { setShutupImportStatus("error"); }
    setTimeout(() => setShutupImportStatus("idle"), 4000);
  };

  const importWinaeroProfile = async () => {
    if (!window.electronAPI?.runScript) {
      toast({ title: "Desktop app required", description: "Requires the desktop .exe app.", variant: "destructive" });
      return;
    }
    setWinaeroImportStatus("launching");
    const script = [
      `$ErrorActionPreference = 'SilentlyContinue'`,
      `$b64 = ''`,
      `$bytes = [System.Convert]::FromBase64String($b64)`,
      `$ini = [System.Text.Encoding]::UTF8.GetString($bytes)`,
      `$dir = Join-Path $env:LOCALAPPDATA 'JGoode-AIO\\Tool'`,
      `if (-not (Test-Path $dir)) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }`,
      `$iniPath = Join-Path $dir 'JGoode-Winaero.ini'`,
      `[IO.File]::WriteAllText($iniPath, $ini)`,
      `$exePath = $null`,
      `$installCandidates = @("$env:ProgramFiles\\Winaero Tweaker\\WinaeroTweaker.exe","\${env:ProgramFiles(x86)}\\Winaero Tweaker\\WinaeroTweaker.exe","$env:LOCALAPPDATA\\Programs\\Winaero Tweaker\\WinaeroTweaker.exe","$env:LOCALAPPDATA\\Winaero Tweaker\\WinaeroTweaker.exe")`,
      `foreach ($rp in @('HKLM:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\WinaeroTweaker.exe','HKCU:\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion\\App Paths\\WinaeroTweaker.exe')) { if (Test-Path $rp) { $v = (Get-ItemProperty $rp -EA SilentlyContinue).'(default)'; if ($v -and (Test-Path $v)) { $exePath = $v; break } } }`,
      `if (-not $exePath) { foreach ($c in $installCandidates) { if (Test-Path $c) { $exePath = $c; break } } }`,
      `if (-not $exePath -and $env:ELECTRON_RESOURCES_PATH) { $setup = Join-Path $env:ELECTRON_RESOURCES_PATH 'executables\\WinaeroTweakerSetup.exe'; if (Test-Path $setup) { Start-Process -FilePath $setup -ArgumentList '/SP-','/VERYSILENT' -Wait; Start-Sleep 2; foreach ($c in $installCandidates) { if (Test-Path $c) { $exePath = $c; break } } } }`,
      `if (-not $exePath) { $ErrorActionPreference = 'Stop'; $wc = New-Object System.Net.WebClient; $wc.Headers.Add('User-Agent','Mozilla/5.0'); $wc.DownloadFile('https://winaerotweaker.com/download/winaerotweaker.zip',($zip=Join-Path $env:TEMP 'WT.zip')); Expand-Archive $zip (Join-Path $env:TEMP 'WT') -Force; $exePath = (Get-ChildItem (Join-Path $env:TEMP 'WT') WinaeroTweaker.exe -Recurse | Select-Object -First 1).FullName }`,
      `if (-not $exePath) { Write-Error 'Not found'; exit 1 }`,
      `Start-Process -FilePath $exePath -ArgumentList "/import:$iniPath" -WindowStyle Normal`,
    ].join("\r\n");
    try {
      const r = await window.electronAPI.runScript(script);
      if (r.success) {
        setWinaeroImportStatus("done");
        toast({ title: "Winaero opened with config", description: "If not auto-applied, use File → Import inside Winaero Tweaker." });
      } else {
        setWinaeroImportStatus("error");
        toast({ title: "Launch failed", description: "Could not open Winaero Tweaker with config.", variant: "destructive" });
      }
    } catch { setWinaeroImportStatus("error"); }
    ...
    <div className="space-y-5 pb-8">
      ...
      <div className="rounded-xl border border-border/50 bg-secondary/10 p-4 space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[12px] font-black text-foreground tracking-tight">Import JGoode's Presets</span>
        ...
        <p className="text-[10px] text-muted-foreground/50 leading-relaxed -mt-0.5">
          One click to auto-import a pre-configured profile directly into each tool, then launch it for review.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2.5">
          <LaunchBtn
            onClick={importCttProfile}
            disabled={cttImportStatus === "running"}
            status={cttImportStatus}
            label="Import CTT Preset"
            icon={FileDown}
            testId="button-import-ctt-preset"
          />
          <LaunchBtn
            onClick={importShutup10Profile}
            disabled={shutupImportStatus === "downloading"}
            status={shutupImportStatus}
            label="Import ShutUp10++ Profile"
            icon={FileDown}
            testId="button-import-shutup10-profile"
          />
          <LaunchBtn
            onClick={importWinaeroProfile}
            disabled={winaeroImportStatus === "launching"}
            status={winaeroImportStatus}
            label="Import Winaero Config"
            icon={FileDown}
            testId="button-import-winaero-config"
          />
        </div>
        {!window.electronAPI && (
          <p className="text-[9.5px] text-muted-foreground/35 text-center">Requires the desktop .exe app</p>
        )}
      </div>
