...
export default function Debloat() {
  ...
  const importWinaeroProfile = async () => {
    ...
    const script = [
      ...
      `Start-Process -FilePath $exePath -ArgumentList "\`"$iniPath\`"" -WindowStyle Normal`,
    ].join("\r\n");
    try {
      const r = await window.electronAPI.runScript(script);
      if (r.success) {
        setWinaeroImportStatus("done");
        toast({ title: "Winaero Tweaker opened", description: "Config saved. If not auto-loaded, use File → Import Settings inside Winaero." });
      } else {
